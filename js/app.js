  ...
  try{
    ...
    try{
      if(mode === "reader"){
        ...
        const total = Number(_effectiveTotalReader() || state.reading.biTotal || 0);
...
function restoreReadingProgress(){
  try{
    ...
    if(pkg && typeof pkg.activeIndex === "number"){
      ...
      const total = Number(_effectiveTotalReader()||0);
...
function renderReader(){
  ...
  // Restore progress for current (bookId + sourceLang + targetLang) when entering the screen
  try{
