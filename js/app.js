...
function loadLastPkg(bookId){
  try{
    const raw = localStorage.getItem(lastPkgKey(bookId));
    if(!raw) return null;
  ...
  try{
    ...
    try{
      if(mode === "reader"){
        ...
        payload.total = total;
        payload.progress = total>0 ? (idx >= (total-1) ? 100 : Math.max(0, Math.min(100, ((idx+1)/total)*100))) : 0;
      }else{
...
function applyLanguagePairChange(){
  ...
  // Goal: when user selects a pair, restore the cursor for this (bookId + sourceLang + targetLang).
  // If pair is new -> start at 0. Never overwrite other pairs here.
  const bookId = resolveBookId();
  const src = String(state.reading.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase();
  const trg = String(state.reading.targetLang || "uk").trim().toLowerCase();

  // Clear translation cache for a clean experience
  try{ state.reading.translateCache.clear(); }catch(e){}

  // Restore saved activeIndex for this pair (shared across modes)
  let idx = 0;
  try{
    const pkg = getPkgProgress(bookId, src, trg);
    if(pkg && Number.isFinite(pkg.activeIndex)) idx = Number(pkg.activeIndex);
  }catch(e){}
  if(!Number.isFinite(idx) || idx < 0) idx = 0;

  // Sync all cursors to the restored index so Play starts correctly.
  ...
  // If we are currently inside a reading route, scroll and highlight immediately.
  try{
    if(state.route?.name === "reader"){
      try{ clearActivePara(); }catch(e){}
      try{ setActivePara(idx); }catch(e){}
      ...
      try{ clearActiveBiLine(); }catch(e){}
      try{ setActiveBiLine(idx); }catch(e){}
      setTimeout(()=>{ try{ scrollToBiLine(idx); }catch(e){} }, 60);
    }
  }catch(e){}
}
/* ---------------------------
...
`;
  ...
  // Prefer last used language pair for this book (so returning from Reader doesn't reset to defaults)
  try{
    const lp = loadLastPkg(b.id);
    if(lp && lp.sourceLang && lp.targetLang){
      state.reading.sourceLang = String(lp.sourceLang).toLowerCase();
      state.reading.targetLang = String(lp.targetLang).toLowerCase();
    }
  }catch(e){}
...
saveReadingProgress(); if(state.reading._browserCancel) state.reading._browserCancel(); }catch(e){}
  ...
  clearActiveWord();
  // keep progress value; don't zero it on stop (would confuse library/history)
  updateProgressUI();
