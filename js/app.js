...
// Effective total lines for progress: last non-empty line + 1 (avoids 99% on trailing blanks)
function _effectiveTotalReader(){
  try{
    const lines = (state.book && state.book.text) ? state.book.text : [];
    ...
    return (state.book && state.book.text) ? state.book.text.length : 0;
  }
}
...
function openFromContinueCard(book){
  if(!book || !book.id){ return; }
  const g = getGlobalLastInteraction();
  let src = null, trg = null, mode = null, idx = 0;
  if(g && g.bookId === book.id){
    src = (g.sourceLang||"").toLowerCase();
    trg = (g.targetLang||"").toLowerCase();
    mode = (g.mode||"").toLowerCase();
    const p = getPkgProgress(book.id, src, trg);
    if(p && typeof p.activeIndex === "number") idx = Math.max(0, Number(p.activeIndex||0));
  }
  if(!src || !trg){
    const last = getLastPkg(book.id);
    if(last){
      src = (last.sourceLang||"en").toLowerCase();
      trg = (last.targetLang||"uk").toLowerCase();
      mode = (last.mode||mode||"listen").toLowerCase();
      const p = getPkgProgress(book.id, src, trg);
      if(p && typeof p.activeIndex === "number") idx = Math.max(0, Number(p.activeIndex||0));
    }
  }
  ...
  state.reading.sourceLang = src;
  state.reading.targetLang = trg;
  const routeName = (mode === "read") ? "bireader" : "reader";
  // set indices upfront (render will restore too)
  try{
    state.reading.activeParaIndex = idx;
    state.reading.resumeIndexReader = idx;
    state.reading.activeBiLineIndex = idx;
  ...
  go({name: routeName, bookId: book.id}, {push:false});
}
...
function restoreReadingProgress(){
  try{
    ...
    if(pkg && typeof pkg.activeIndex === "number"){
      ...
      const total = effectiveTotalLines(state.book?.text||[]);
      state.reading.progress = total>0 ? (Math.min(idx, total-1)+1)/total : 0;
...
function applyLanguagePairChange(){
  // Do NOT save/stop/reset here — callers already do Stop→Save→Change.
  // Just restore progress for current (bookId + src + trg) and sync cursors.
  const idx = (()=>{
...
if(route.name === "details"){
    // Keep cursor indices; progress is restored inside render*()
...
function renderCatalog(){
  ...
  if(cont){
    const openCont = ()=>{ try{ openFromContinueCard(cont); }catch(e){ go({name:"details", bookId: cont.id}); } };
...
function renderReader(){
  ...
  updateProgressUI();
  // Restore progress for current book+language pair
  try{ const __idx = restoreReadingProgress(); setActivePara(__idx); }catch(e){}
...
function renderBiReader(){
  ...
  // Restore progress for current book+language pair
  try{ const __idx = restoreReadingProgress(); setActiveLineUI(__idx); }catch(e){}
...
const playNext = async ()=>{
    ...
    try{
      const total = (mode === 'reader') ? _effectiveTotalReader() : (state.reading.biTotal || (lines.length||0));
      if(total > 0){
        state.reading.progress = Math.min(1, (openaiLineIndex + 1) / total);
        updateProgressUI();
      }
    }catch(e){}
...
function stopReading({save=true} = {}){
  try{
    // remember last position per mode (prefer UI cursor; openaiLineIndex can be stale after navigation)
    try{
      const mode = state.route?.name;
      if(mode === 'bireader'){
        const a = Number(state.reading.activeBiLineIndex);
        const r = Number(state.reading.resumeIndexBi);
        const oi = Number.isFinite(openaiLineIndex) ? Number(openaiLineIndex) : NaN;
        let idx = Number.isFinite(a) ? a : (Number.isFinite(r) ? r : (Number.isFinite(oi) ? oi : 0));
    ...
    if(state.reading._browserCancel) try{ state.reading._browserCancel(); }catch(e){}
    state.reading._browserCancel = null;

    // Stop OpenAI playback
    openaiStopRequested = true;
    state.reading.isPlaying = false;
    try{ btnPlay && (btnPlay.textContent = "▶"); }catch(e){}
    try{
      if(openaiAudio){
        try{ openaiAudio.pause(); }catch(e){}
        try{ openaiAudio.src = ""; }catch(e){}
      }
    }catch(e){}
  }catch(e){}
}
...
function finishReading(){
  try{
    // Move cursor to the last meaningful line so % becomes 100
    const lines = getListenLines() || [];
    let last = -1;
    for(let i = lines.length - 1; i >= 0; i--){
      if(String(lines[i] ?? "").trim()){ last = i; break; }
    }
    if(last < 0) last = Math.max(0, lines.length - 1);

    openaiLineIndex = last;
    if(state.route?.name === "reader"){
      try{ setActivePara(last); }catch(e){}
      state.reading.resumeIndexReader = last;
    }else if(state.route?.name === "bireader"){
      try{ setActiveLineUI(last); }catch(e){}
      state.reading.resumeIndexBi = last;
    }
