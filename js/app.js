...
function applyLanguagePairChange(){
  // IMPORTANT: callers should stop+save BEFORE changing (sourceLang/targetLang).
  // Here we only RESTORE the cursor for the current (bookId + sourceLang + targetLang),
  // and sync all cursors (so Read/Listen share the same position).
  let idx = 0;
  try{
    const bookId = resolveBookId();
    const src = String(state.reading.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase();
    ...
    const p = getPkgProgress(bookId, src, trg);
    if(p && Number.isFinite(Number(p.activeIndex))){
      idx = Math.max(0, Number(p.activeIndex));
    }else{
      // fallback to legacy per-mode session progress
      idx = Math.max(0, Number(restoreReadingProgress() || 0));
    }
  }catch(e){
    try{ idx = Math.max(0, Number(restoreReadingProgress() || 0)); }catch(_e){ idx = 0; }
  }

  // sync all internal cursors
  ...
  // repaint current view if we are inside reader screens
  try{
    if(state.route?.name === "reader"){
      try{ clearActivePara(); }catch(e){}
      try{ setActivePara(idx); }catch(e){}
...
setLabels();
  ...
  src.onchange = ()=>{
    // stop + save CURRENT pair first, then switch pair, then restore for the new pair
    try{ stopReading(); }catch(e){}
...
function finishReading(){
  ...
  // Sync cursor to the LAST non-empty line and persist 100% progress
  try{
    const total = Number(state.book?.text?.length || 0);
    const last = total>0 ? (total-1) : 0;
    if(Number.isFinite(last) && last>=0){
      openaiLineIndex = last;
      if(state.route?.name === "reader"){ state.reading.activeParaIndex = last; state.reading.resumeIndexReader = last; }
      if(state.route?.name === "bireader"){ state.reading.activeBiLineIndex = last; state.reading.resumeIndexBi = last; }
      try{ saveReadingProgress(); }catch(e){}
    }
  }catch(e){}
