...
function jumpToChapter(chIdx){
  ...
  // Stop playback (and save current position) BEFORE jumping
  try{ stopReading({save:true}); }catch(e){}

  if(state.route?.name === 'reader'){
    // Jump inside Listen screen
    try{ setActivePara(idx); }catch(e){ try{ state.reading.activeParaIndex = idx; state.reading.resumeIndexReader = idx; openaiLineIndex = idx; }catch(_e){} }
    try{ saveReadingProgress(); }catch(e){}
    return;
  }
  if(state.route?.name === 'bireader'){
    // Jump inside Read screen
    try{ setActiveLineUI(idx); }catch(e){ try{ state.reading.activeBiLineIndex = idx; state.reading.resumeIndexBi = idx; openaiLineIndex = idx; }catch(_e){} }
    try{ saveReadingProgress(); }catch(e){}
    return;
  }
...
function effectiveTotalLines(lines){
  // last non-empty line index + 1 (so trailing blanks don't cap progress at 99%)
  try{
    for(let i=(lines||[]).length-1;i>=0;i--){
      const s = String(lines[i] ?? "");
      if(s.trim()) return i+1;
    }
  }catch(e){}
...
function clearActivePara(){
  try{
    // remove ALL paragraph highlights to avoid 'stuck' lines after fast interactions
    document.querySelectorAll('.activeLine').forEach(el=>el.classList.remove('activeLine'));
  }catch(e){}
  try{
    // remove any lingering paragraph word highlights
    const pw = state.reading.paraWords || {};
    Object.keys(pw).forEach(k=>{ (pw[k]||[]).forEach(sp=>sp.classList.remove("active")); });
  }catch(e){}
...
function setActivePara(idx){
  // IMPORTANT: always sync playback cursor with UI cursor
  idx = Number(idx);
...
function finishReading(){
  ...
  // clear highlights (prevents last-word 'stuck' in Listen)
  try{ clearAllWordHighlights(); }catch(e){}
  try{ clearActiveWord(); }catch(e){}
  try{ clearActiveLineUI(); }catch(e){}
  try{ clearActivePara(); }catch(e){}
