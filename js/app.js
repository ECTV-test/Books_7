...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    // entering Listen screen: stop any playback but DO NOT reset cursor indices
    stopReading();
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      state.book = book;
      renderReader();
  ...
  if(route.name === "bireader"){
    // entering Read screen: stop any playback but DO NOT reset cursor indices
    stopReading();
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      state.book = book;
      renderBiReader();
  ...
  if(cont){
    const openCont = ()=>{
      try{
        const last = getLastPkg(cont.id);
        if(last){
          // apply last used language pair + mode
          ...
          const routeName = (last.mode === "read") ? "bireader" : "reader";
          const lp = getPkgProgress(cont.id, state.reading.sourceLang, state.reading.targetLang);
          const startIndex = lp && typeof lp.activeIndex === "number" ? Number(lp.activeIndex||0) : 0;
          return go({name: routeName, bookId: cont.id, startIndex}, {push:false});
        }
      }catch(e){}
      return go({name:"details", bookId: cont.id});
    };
...
function renderDetails(){
  ...
  // Restore last used language pair for this book (so re-opening Details won't fall back to defaults)
  try{
    const lastPkg = getLastPkg(b.id);
    if(lastPkg){
      state.reading.sourceLang = lastPkg.sourceLang || state.reading.sourceLang;
      state.reading.targetLang = lastPkg.targetLang || state.reading.targetLang;
    }
  }catch(e){}
...
function stopReading(){
  try{
    ...
    try{
      const mode = state.route?.name;
      if(mode === 'bireader'){
        const ui = Number(state.reading.activeBiLineIndex);
        const idx = (Number.isFinite(ui) && ui >= 0) ? ui : (Number.isFinite(openaiLineIndex) ? openaiLineIndex : 0);
        state.reading.resumeIndexBi = idx;
      }else if(mode === 'reader'){
        const ui = Number(state.reading.activeParaIndex);
        const idx = (Number.isFinite(ui) && ui >= 0) ? ui : (Number.isFinite(openaiLineIndex) ? openaiLineIndex : 0);
        state.reading.resumeIndexReader = idx;
      }
    }catch(e){}
