...
function _lastNonEmptyIndex(arr){
  try{
...
function _effectiveTotalReader(){
  const arr = state.book?.text || [];
  const last = _lastNonEmptyIndex(arr);
  return last >= 0 ? (last + 1) : (arr.length || 0);
}
function _effectiveTotalListen(){
  const arr = getListenLines() || [];
  const last = _lastNonEmptyIndex(arr);
  return last >= 0 ? (last + 1) : (arr.length || 0);
}
...
function saveReadingProgress(){
  try{
    ...
    try{
      if(mode === "reader"){
        const total = Number(_effectiveTotalReader() || state.reading.totalParas || 0);
        const lastIdx = Math.max(0, total-1);
        const idx0 = Number.isFinite(payload.activeParaIndex) ? payload.activeParaIndex : 0;
        const idx = Math.max(0, Math.min(lastIdx, idx0));
        payload.total = total;
        payload.progress = total>0 ? Math.max(0, Math.min(100, ((idx+1)/total)*100)) : 0;
      }else{
        const total = Number(_effectiveTotalListen() || state.reading.biTotal || state.book?.text?.length || 0);
        const lastIdx = Math.max(0, total-1);
        const idx0 = Number.isFinite(payload.activeBiLineIndex) ? payload.activeBiLineIndex : 0;
        const idx = Math.max(0, Math.min(lastIdx, idx0));
        payload.total = total;
        payload.progress = total>0 ? Math.max(0, Math.min(100, ((idx+1)/total)*100)) : 0;
      }
...
async function startReadingOpenAI(){
  ...
  const mode = state.route?.name;
  if(mode === "bireader"){
    // IMPORTANT: start from UI cursor first (activeBiLineIndex), then fallback to resumeIndexBi
    openaiLineIndex = Number.isFinite(state.reading.activeBiLineIndex) ? state.reading.activeBiLineIndex :
      (Number.isFinite(state.reading.resumeIndexBi) ? state.reading.resumeIndexBi : 0);
  }else{
    // IMPORTANT: start from UI cursor first (activeParaIndex), then fallback to resumeIndexReader
    openaiLineIndex = Number.isFinite(state.reading.activeParaIndex) ? state.reading.activeParaIndex :
      (Number.isFinite(state.reading.resumeIndexReader) ? state.reading.resumeIndexReader : 0);
  }
...
function stopReading(){
  try{
    ...
    saveReadingProgress();
    if(state.reading._browserCancel) state.reading._browserCancel();
  }catch(e){}
...
function finishReading(){
  ...
  // Force completion to 100% for this (book + language pair)
  try{
    const mode = state.route?.name;
    if(mode === "reader"){
      const total = Number(_effectiveTotalReader() || state.book?.text?.length || 0);
      const lastIdx = Math.max(0, total-1);
      state.reading.activeParaIndex = lastIdx;
      state.reading.resumeIndexReader = lastIdx;
      openaiLineIndex = lastIdx;
    }else if(mode === "bireader"){
      const total = Number(_effectiveTotalListen() || (getListenLines()||[]).length || 0);
      const lastIdx = Math.max(0, total-1);
  ...
  // Save progress now so Library can move it to Finished immediately
  try{ saveReadingProgress(); }catch(e){}
