...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      // keep narration cursor in sync with UI cursor
...
async function startReadingOpenAI(){
  ...
  if(mode === "bireader"){
    const a = Number(state.reading.activeBiLineIndex);
    const r = Number(state.reading.resumeIndexBi);
    // Prefer the UI cursor (active*) when it is non-zero; resume* can be reset on navigation.
    if(Number.isFinite(a) && a > 0) openaiLineIndex = a;
    else if(Number.isFinite(r) && r > 0) openaiLineIndex = r;
    else openaiLineIndex = (Number.isFinite(a) ? a : (Number.isFinite(r) ? r : 0));
  }else{
    const a = Number(state.reading.activeParaIndex);
    const r = Number(state.reading.resumeIndexReader);
    if(Number.isFinite(a) && a > 0) openaiLineIndex = a;
