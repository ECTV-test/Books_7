...
function addBookmarkEntry({bookId, paraIdx, raw, tr, sourceLang, targetLang, mode}){
  ...
  const sLang = String(sourceLang||state.reading?.sourceLang||state.book?.sourceLang||"en").trim().toLowerCase();
  const tLang = String(targetLang||state.reading?.targetLang||"uk").trim().toLowerCase();
  const m = String(mode||pkgMode(state.route?.name)||state.reading?.mode||"").trim().toLowerCase();
  const entry = {
...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      // If bookmark jump provided startPara, it must win (and also work in Listen mode).
      const __sp = (route.startPara != null) ? Number(route.startPara) : NaN;
      if(Number.isFinite(__sp) && __sp >= 0) idx = __sp;
      // If startIndex is provided (mode switch / bookmark jump) it must win.
      const __routeStart = Number(route.startIndex);
      if(Number.isFinite(__routeStart) && __routeStart >= 0) idx = __routeStart;

      try{ clearAllWordHighlights(); }catch(e){}
      try{ clearActivePara(); }catch(e){}
      try{ syncCursorIndex(idx); }catch(e){}
      try{ setActivePara(idx); }catch(e){ try{ state.reading.activeParaIndex = idx; state.reading.resumeIndexReader = idx; }catch(_e){} }
      setTimeout(()=>{ try{ scrollToPara(idx); }catch(e){} }, 60);

      // Persist this jump as the new resume point for this language pair
      try{ saveReadingProgress(); }catch(e){}

      // Prevent accidental reuse on subsequent navigations
      try{ state.route.startIndex = undefined; }catch(e){}
      try{ state.route.startPara = undefined; }catch(e){}
    });
  }
  ...
  if(route.name === "bireader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      showPlayer();
      let idx = (function(){ try{ return restoreReadingProgress()||0; }catch(e){ return 0; } })();
      // If bookmark jump provided startPara, it must win.
      const __sp = (route.startPara != null) ? Number(route.startPara) : NaN;
      if(Number.isFinite(__sp) && __sp >= 0) idx = __sp;
      // If startIndex is provided (mode switch / bookmark jump) it must win.
      const __routeStart = Number(route.startIndex);
...
function renderLibrary(){
  ...
  app.innerHTML = `
    <div class="wrap">
      ...
      ${tab==="bookmarks" && state.ui?.bookmarksReturn?.bookId ? `
        <div style="padding: 0 18px 10px 18px;">
          <button class="btn" id="btnBackToBook" style="width:100%;justify-content:center;gap:10px;">
            ↩︎ Back to book
          </button>
        </div>
      ` : ``}
  ...
  // Back to the exact book + language pair + resume index
  const __btb = document.getElementById("btnBackToBook");
  if(__btb){
    __btb.addEventListener("click", (e)=>{
      e.preventDefault();
      e.stopPropagation();
      const ret = state.ui?.bookmarksReturn;
      if(!ret || !ret.bookId) return;
      try{
        state.reading.sourceLang = ret.sourceLang || state.reading.sourceLang;
        state.reading.targetLang = ret.targetLang || state.reading.targetLang;
      }catch(_e){}
      const idx = Math.max(0, Number((ret.resumeIndex ?? ret.index) || 0));
      const mode = String(ret.mode||"listen").toLowerCase();
      // Consume return so it doesn't stick forever.
      try{ delete state.ui.bookmarksReturn; }catch(_e){}
      if(mode === "read") go({name:"bireader", bookId: ret.bookId, startIndex: idx, startPara: idx}, {push:false});
      else go({name:"reader", bookId: ret.bookId, startIndex: idx, startPara: idx}, {push:false});
    });
  }
  ...
  if(tab==="bookmarks"){
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        // Save a clean return point (resume) BEFORE jumping to a bookmark.
        // If Bookmarks was opened from a book, keep that original return target.
        try{
          state.ui = state.ui || {};
          if(!state.ui.bookmarksReturn){
            const s = String(it?.sourceLang || getLastPkg(bookId)?.sourceLang || state.reading.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase();
            const t = String(it?.targetLang || getLastPkg(bookId)?.targetLang || state.reading.targetLang || "uk").trim().toLowerCase();
            const prev = getPkgProgress(bookId, s, t);
            const prevIdx = (prev && typeof prev.activeIndex === "number") ? Math.max(0, Number(prev.activeIndex||0)) : 0;
            state.ui.bookmarksReturn = {
              bookId,
              sourceLang: s,
              targetLang: t,
              mode: (it?.mode || getLastPkg(bookId)?.mode || "listen"),
              resumeIndex: prevIdx,
              ts: Date.now()
            };
          }
        ...
        // Apply pair from the bookmark (if present)
        try{
          if(it?.sourceLang) state.reading.sourceLang = String(it.sourceLang).trim().toLowerCase();
          if(it?.targetLang) state.reading.targetLang = String(it.targetLang).trim().toLowerCase();
        }catch(_e){}

        const m = String(it?.mode || "listen").trim().toLowerCase();
        if(m === "read") go({name:"bireader", bookId, startIndex: idx, startPara: idx}, {push:false});
        else go({name:"reader", bookId, startIndex: idx, startPara: idx}, {push:false});
      });
    });
...
function renderReader(){
  ...
  if(__tbm){
    __tbm.addEventListener("click", (e)=>{
      ...
      // Remember where we came from so Library(Bookmarks) can offer "Back to book"
      try{
        const bookId = resolveBookId();
        const src = String(state.reading.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase();
        const trg = String(state.reading.targetLang || "uk").trim().toLowerCase();
        const idx = Number.isFinite(state.reading.cursorIndex) ? Number(state.reading.cursorIndex) : (Number.isFinite(openaiLineIndex) ? Number(openaiLineIndex) : 0);
        state.ui = state.ui || {};
        state.ui.bookmarksReturn = { bookId, sourceLang: src, targetLang: trg, mode: pkgMode(state.route?.name), resumeIndex: Math.max(0, idx), ts: Date.now() };
      }catch(_e){}
      state.ui = state.ui || {};
      state.ui.libraryTab = "bookmarks";
...
async function translateWord(word){
  ...
  // Use the SAME provider switch as line translation (Admin → LibreTranslate/OpenAI).
  // Cache separated from line translation to avoid collisions.
  const provider = String(state.dev.translationProvider || "libre").toLowerCase();
  const cacheKey = `WORD|${provider}|${state.reading.targetLang}|${clean}`;
  if(state.reading.translateCache.has(cacheKey)) return state.reading.translateCache.get(cacheKey);

  // If OpenAI is selected, reuse the Worker translation (cached server-side).
  if(provider === "openai"){
    const translated = await translateTextAny(clean);
